...
function fullName(peopleArr) {
  ...
  peopleArr.forEach((item) => {
    if (item.name.middle === undefined) {
      result.push(`${item.name.first} ${item.name.last}`);
    } else {
      result.push(`${item.name.first} ${item.name.middle} ${item.name.last}`);
    }
  });
  return result;
}
